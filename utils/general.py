...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    """
    print("agnostic is", agnostic, "conf_thres", conf_thres, "iou_thres", iou_thres)
...
def postprocess_hu(output, grid, model):

    if hasattr(model, "head"):
        det = model.head
    else:
        det = model.model[-1]
    nl = det.anchors.shape[0]
    z = []
    anchor_grid = det.anchor_grid
    model_stride = torch.Tensor([16,32])
    for i in range(nl):
        bs, ny, nx, no = output[i].shape[0], output[i].shape[2], output[i].shape[3], output[i].shape[4]

        if grid[i].shape[2:4] != output[i].shape[2:4]:
            grid[i] = _make_grid(nx, ny).to(output[i].device)

        y = output[i].sigmoid()
        y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + grid[i].to(output[i].device)) * model_stride[i].to(output[i].device)  # xy
...
# Rename state dict keys to match quantized yolov5 architecture for submitted version
def rename_state_dict_keys(state_dict):
    new_state_dict = OrderedDict()
    for k, v in state_dict.items():
        if v.shape == torch.Size([]):   # skip bn.num_batches_tracked
            continue
...
# x is fp32, y is dequant int8
def compute_error(x, y):
    Ps = torch.norm(x)
    Pn = torch.norm(x-y)
    return 20*torch.log10(Ps/Pn)

# Rename state dict keys to match quantized yolov5 architecture for original YOLOv5
def rename_state_dict_keys_origin(state_dict):
    new_state_dict = OrderedDict()
    for k, v in state_dict.items():
        if v.shape == torch.Size([]):   # skip bn.num_batches_tracked
            continue
        ...
        if k[6] == '0':
            ...
            #if k[-7:] != 'anchors' and k[-4:] != 'grid':
            name = 'head' + k[8:]
                #name = name.replace('m.0', 'cv1')
                #name = name.replace('m.1', 'cv2')
                #name = name.replace('m.2', 'cv3')
            new_state_dict[name] = v
